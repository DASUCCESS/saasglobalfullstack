...
export default function Page() {
  ...
  const save = async () => {
    ...
    const res = await apiPost(
      ...
      { contact: { ...form, smtp_port: Number(form.smtp_port || 587) } },
      token
    );
    if (!res) {
  ...
  return (
    <AdminShell title="SMTP">
      <div className="max-w-3xl rounded-xl border border-neutral-800 bg-neutral-900 p-5 grid gap-3">
        ...
        <SecretInput value={form.smtp_password} onChange={(value) => setForm({ ...form, smtp_password: value })} placeholder="SMTP password" />
        ...
        <label className="text-sm">
          <input type="checkbox" checked={form.smtp_use_tls} onChange={(e) => setForm({ ...form, smtp_use_tls: e.target.checked })} />
          <span className="ml-2">Use TLS</span>
        </label>

...
import { ArrowUp, X, ChevronRight, Bot } from "lucide-react";
  ...
  return (
    <>
      <div className="fixed right-6 bottom-6 z-50 flex flex-col items-end gap-3">
        <a href={aiUrl} className="group inline-flex items-center gap-2 rounded-full px-5 py-3 bg-black text-white font-medium shadow-xl hover:shadow-2xl transition-transform duration-200 hover:scale-105 cursor-pointer">
          <Bot className="w-4 h-4" />
          <span>{aiLabel}</span>
        </a>

        <button onClick={() => setOpen(true)} className="group inline-flex items-center gap-2 rounded-full px-5 py-3 bg-brand-yellow text-black font-medium shadow-xl hover:shadow-2xl transition-transform duration-200 hover:scale-105 cursor-pointer">
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" role="dialog" aria-modal="true">
          ...
          <div className="relative w-[92%] max-w-2xl rounded-2xl bg-white p-6 shadow-2xl max-h-[85vh] flex flex-col">
